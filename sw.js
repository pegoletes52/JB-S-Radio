  ...
  event.waitUntil(
    caches.open(CACHE_NAME).then(function (cache) {
      return cache.addAll(SHELL_FILES).catch(function (err) {
        // Si el cacheo del cascarón falla por cualquier motivo, no bloquea la
        // instalación del service worker (que es lo que habilita "Instalar app").
        console.warn("No se pudo cachear el cascaron:", err);
      });
    })
  );
  self.skipWaiting();
